...
//        individualExecuteOptimize(target);
    ...
    /**
     * 思路：双指针移动，x从数组最左侧开始向右移动，y从数组最右侧开始向左移动。移动过程中相当于不断计算最矮边界的最大面积，等遍历了所有的边界之后，最大面积即可获得
     * 步骤：当确定两条线时，分别用x指针和y指针指向这两条线，当前面积取决于高度最矮的那条线，且该面积就是最矮那条线所能产生的最大面积(最大高度就是最短的那条线，长度不可能比边界还长)。
     * 假设如果我们不移动最矮的那条线，那么移动过程中x轴的值一定变小，由于最矮的那条线没动，所以y轴最大值只能是最矮的那条线，此移动无意义。因此我们肯定移动最矮的那条线，虽然
