    ...
    private static List<List<Integer>> standardExecute(int[] nums) {
        ...
        for (int a = 0; a < nums.length; a++) {
            ...
            while (b < c) {
                 ...
                 * 或者右指针向左移动时，如果数字和前一个相同，则移至下一位。c起始位置不需要判断，因为刚起步没有前一个值
                 * 4.2、如果三数之和小于0，由于是升序数组，因此想要结果为0，只能向右移动b，提升整体加和的结果。
                 * 或者左指针向右移动时，如果数字和前一个相同，则移至下一位。b起始位置不需要判断，因为刚起步没有前一个值
                 * 4.3、如果三数之和等于0，则找到结果放入结果集中。左指针右移，右指针左移。
                 */
                if (nums[a] + nums[b] + nums[c] > 0 || c < nums.length - 1 && nums[c] == nums[c + 1]) {
                    c--;
                } else if (nums[a] + nums[b] + nums[c] < 0 || b > a + 1 && nums[b] == nums[b - 1]) {
                    b++;
                } else {
