 ...
 * ！！！！！！！！！因此动态规划通常会额外创建一个结构相同的数据结构，用于存储对应位置的最优解！！！！！！！！！！！！
 * 但是本题不用，因为本题中我们只需要在当前网格中存储即可，因为我们最终求得是累计和，因此每个网格都存储当前累计和的结果，这样当前点计算最优值时
 * 直接将自己的值和相邻的最优网格值加和即当前网格的最优解，降低空间复杂度
 * 拿本题“找网格的最短路径”举例：
 * 题干要求每次移动只能向右或者向下，我们想找到终点网格的最优解(记为dp(终点))，如果我们能找到终点网格的上面网格或左面网格对应的最优解dp(终点-上)和dp(终点-左)，
 * 那么我们只需要在dp(终点-上)和dp(终点-左)中找到最小的值，最终加上终点网格的值就是最短路径，即min(dp(终点-上),dp(终点-左))+dp(当前网格)；
 * 那么如果我们想找到dp(终点-上)和dp(终点-左)，我们只需要找到dp(终点-上-上)和dp(终点-上-左)的最小值，以及dp(终点-左-上)和dp(终点-左-左)的最小值，依此类推。
 * 最终我们发现解决方案其实就是找到每个网格的左和上网格的最优解，二者取最优然后和当前网格加和就是当前网格的最优解。即每个网格最优解的
 * 公式都是：min(dp(当前网格的上网格),dp(当前网格的左网格))+dp(当前网格);
 * 如果从起点出发由于很多边界网格只有一种走法，因此很多网格的最优解很好确认，这样不断遍历每个网格，就能把每个网格的最优解都计算出来，最终到达最后一个网格我们就可以得出最终的最优解。
 * <p>
 * 总结：动态规划解决决策问题(每一步有多种选择的问题)时，其实就是先把能到达当前步骤的之前步骤的最优解找到，然后从中选择最优再和当前步骤进行结合，就能找到当前步骤的最优解
 * 动态规划基本问题中一定存在边界(只有一种选择)，这样从这些点出发就很容易将全部步骤的最优解都找到！！！！！！！！！！！！！！！！！！
 * <p>
 * 二、二维数组
...
//        int[][] grid = {{1,3,1},{1,5,1},{4,2,1}};
    ...
    /**
     * 思路：
     * 1、MARK中已经提到，根据动态规划，我们可以将问题“找到最终网格的最优解”拆分为“找到可抵达当前网格的每个网格的最优解”。能到达当前
     *      网格的只有该网格的上边和左边的网格。到达上边和左边网格的最优解又是这两个网格分别对应的左边和上边网格的最优解~依此类推...
     *      当抵达网格边界时，由于只有一种选择，因此对应网格的最优值可确定，这样相邻网格的最优值也可确定，最终可确定终点网格的最优解
     * 2、我们从(0,0)起始点出发，按行或者按列移动均可，我们从行出发~，找到到达每个点的最优解。
     *
     * @param grid 二维数组
     * @return 返回最小路径和，起始最后一个网格的值
     */
        ...
        for (int x = 0; x < grid.length; x++) {
            for (int y = 0; y < grid[x].length; y++) {
                if (x == 0 && y == 0) {
                    ...
                    //第一行处于边界，只能从左边往右走，因此最优解就是左侧的值和当前值的和。
                    grid[x][y] = grid[x][y - 1] + grid[x][y];
                } else if (y == 0) {
                    //第一列处于边界，只能从上边往下走，因此最优解就是上边的值和当前值的和。
                    grid[x][y] = grid[x - 1][y] + grid[x][y];
                } else {
                    // 每个网格的最优解都是上边和左边网格的最优解的最小值和当前网格值得和
